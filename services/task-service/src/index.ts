// ─────────────────────────────────────────────────────────────
// LastMinute — Task Service Entry Point
...
app.get("/health", async (_req, res) => {
  try {
    ...
    res.status(500).json({ status: "DOWN", service: "task-service" });
  }
});
...
async function startServer() {
  ...
  app.listen(config.port, () => {
    logger.info(`Task Service listening on port ${config.port} [ENV: ${config.env}]`);
  });
}
