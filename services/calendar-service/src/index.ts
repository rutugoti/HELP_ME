// ─────────────────────────────────────────────────────────────
// LastMinute — Calendar Service Entry Point
...
app.get("/health", async (_req, res) => {
  try {
    ...
    res.status(200).json({ status: "UP", service: "calendar-service" });
  } catch {
    res.status(500).json({ status: "DOWN", service: "calendar-service" });
  }
});
...
async function startServer() {
  ...
  app.listen(config.port, () => {
    logger.info(`Calendar Service listening on port ${config.port} [ENV: ${config.env}]`);
  });
}
